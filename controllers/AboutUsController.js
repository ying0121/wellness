...
exports.render = async (req, res, next) => {
	...
	//
	data.page_tag = "about-us"
	data.page_title = "THE HEIGHTS WELLNESS - About Us"
	...
	const _header_banner = await PageImg.findAll({ attributes: attributes, where: { status: 1, page: "AboutUs", position: "HEADER-BANNER" } })
	data.HEADER_BANNER = []
	_header_banner.forEach(item => { data.HEADER_BANNER.push(item.dataValues) })

    const _central = await PageImg.findAll({ attributes: attributes, where: { status: 1, page: "AboutUs", position: "CENTRAL" } })
	data.CENTRAL = []
	_central.forEach(item => { data.CENTRAL.push(item.dataValues) })
	...
	attributes = [
        ...
        'denominator',
        'numerator',
        [siteLang === 'en' ? 'desc_en' : 'desc_es', 'mdesc'],
        [siteLang === 'en' ? 'fdesc_en' : 'fdesc_es', 'mfdesc'],
        'sdate',
        'edate'
    ]

	data.qualities = await Measures.findAll({
		attributes: attributes,
		include: [{
			model: QualityCat,
			attributes: [[Sequelize.col(siteLang === 'en' ? 'en_name' : 'es_name'), 'catname']],
			where: { status: 1 }, // quality_cats.status = 1
			required: true        // INNER JOIN (not LEFT JOIN)
		}],
		where: {
			status: 1 // measures.status = 1
		},
		order: [['catid', 'ASC']],
		raw: true,
		nest: true
