...
exports.render = async (req, res, next) => {
	...
	//
	data.page_tag = "orientation"
	data.page_title = "THE HEIGHTS WELLNESS - Orientation"
    ...
    const _documents = await Document.findAll({ attributes: attributes, where: { page: "Orientation" } })
    data.documents = []
    _documents.forEach(item => { data.documents.push(item.dataValues) })
