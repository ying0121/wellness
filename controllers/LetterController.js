...
exports.render = async (req, res, next) => {
	...
	//
	data.page_tag = "letters"
	data.page_title = "THE HEIGHTS WELLNESS - Letter"
	...
	data.letters = await Letters.findAll({ where: { language: siteLang == "en" ? 17 : 25 }, include: [{ model: LetterCategories, as: "LetterCategories", attributes: ["name"] }] })
...
exports.renderDetail = async (req, res, next) => {
	...
	data.letter = await Letters.findOne({ where: { id: req.query.s } })

	//
	data.page_tag = "letter-detail"
	data.page_title = "THE HEIGHTS WELLNESS - Letter - " + data.letter.title
	data.page_desc = "The Heights Wellness Medical Service"
	data.site_url = process.env.SITE_URL
	data.og_title = "The Heights Wellness"
	data.og_desc = "The Heights Wellness Medical Service"
	...
	// QR Code
	let qrcode_text = process.env.SITE_URL +  "/letters/detail?s=" + req.query.s
...
exports.submitForLetter = async (req, res, next) => {
	...
	if (req.session.ying_letter === md5(md5(md5(captcha)))) {
		req.session.ying_letter = null
