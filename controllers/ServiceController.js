...
exports.render = async (req, res, next) => {
	...
	//
	data.page_tag = "services"
	data.page_title = "THE HEIGHTS WELLNESS - Service"
...
exports.renderDetail = async (req, res, next) => {
	...
	data.page_title = "THE HEIGHTS WELLNESS - Service - " + data.service.title
	data.page_desc = "The Heights Wellness Medical Service"
	data.site_url = process.env.SITE_URL
	data.og_title = "The Heights Wellness"
	data.og_desc = "The Heights Wellness Medical Service"
	...
	const friend_captcha = await getCaptcha(6, 2, false)
	data.friend_captcha_image = friend_captcha.captcha
	req.session.ying_friend = friend_captcha.key
	...
	// QR Code
	let qrcode_text = process.env.SITE_URL +  "/services/detail?s=" + req.query.s
	data.qrcode = await generateQRCode(qrcode_text)

	// Footer QR Code
	qrcode_text = `NAME : ${data.contact_info.name} \n`
	qrcode_text += `EMAIL : ${data.contact_info.email} \n`
	qrcode_text += `SITE : ${process.env.SITE_URL} \n`
	qrcode_text += `ADDRESS : ${data.contact_info.address} \n`
	qrcode_text += `CITY : ${data.contact_info.city} \n`
...
exports.getCost = async (req, res, next) => {
	const cost = await ClinicService.findOne({ attributes: ["cost"], where: { id: req.body.id } })
	res.status(200).json({ cost: cost.cost })
}

exports.submitForService = async (req, res, next) => {
	...
	if (req.session.ying_service === md5(md5(md5(captcha)))) {
		req.session.ying_service = null
			...
			if (error) {
				...
				transporter.sendMail(mailOptions, (error, info) => {
					if (error) {
						...
						return res.status(200).json({ status: "success", id: case_number })
					}
				})
			}
		})
...
exports.sendEmailToFriend = async (req, res, next) => {
	const contactInfo = await ContactInfo.findOne({ where: { id: 1 } })
	const acronym = contactInfo.acronym
	const privateKey = process.env.ENCRYPT_PRIVATE_KEY
	...
	if (req.session.ying_friend === md5(md5(md5(captcha)))) {
		req.session.ying_friend = null

		const translations = await Translation.findAll({ where: { keyvalue: { [Op.like]: "t_sf_%" } } })
		let component_text = []
		translations.forEach(item => { component_text[item.keyvalue] = item[req.session.language] })

		const service = await ClinicService.findOne({ where: { id: id } })

		let text = component_text.t_sf_updated_text
		text = text.replace(/\$sender_name/g, name)
		text = text.replace(/\$clinic_name/g, contactInfo.name)
		text = text.replace(/\$service_name/g, service.title)
		text = text.replace(/\$short_desc/g, service.short_desc)

		// send email begin //
		const emailSubject = component_text.t_sf_subject
		...
		ejs.renderFile(path.join(__dirname, '../views/email/friendemail.ejs'), emailConfig, (error, data) => {
			if (error) {
				return res.status(200).json({ status: "error", message: "mail" })
			} else {
				const mailOptions = {
